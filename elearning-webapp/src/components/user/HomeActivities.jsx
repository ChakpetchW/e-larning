...
const HomeActivities = ({ courses, goalsProgress, onNavigate }) => {
  return (
    <div className="animate-slide-up" style={{ animationDelay: '300ms' }}>
      <h3 className="text-xl md:text-2xl font-bold text-slate-800 tracking-tight mb-5 px-1 md:px-2">เธเธดเธเธเธฃเธฃเธกเธเธฒเธฃเน€เธฃเธตเธขเธ</h3>
      <div className={`grid gap-5 md:gap-8 ${goalsProgress.length <= 1 ? 'grid-cols-1 md:grid-cols-2 items-center' : 'grid-cols-1'}`}>
        ...
        >
          ...
          <div className="flex-1">
            <p className="text-[11px] text-slate-400 font-bold uppercase tracking-widest mb-1">เธเธณเธฅเธฑเธเน€เธฃเธตเธขเธเธญเธขเธนเน</p>
            <h3 className="text-2xl font-black text-slate-800 leading-none">
              {courses.filter((course) => course.isEnrolled && course.enrollmentStatus === ENROLLMENT_STATUS.IN_PROGRESS).length} เธเธญเธฃเนเธช
            </h3>
          </div>
          ...
          {goalsProgress.length > 0 ? (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded border leading-none ${gp.scope === 'DEPARTMENT' ? 'border-warning-border text-warning-text bg-warning-bg' : 'border-info-border text-info-text bg-info-bg'}`}>
                      {gp.scope === 'DEPARTMENT' ? (gp.deptName || 'เนเธเธเธ') : 'เธญเธเธเนเธเธฃ'}
                    </span>
                  </div>
                  <h3 className="text-xl font-bold text-slate-800 leading-none mb-3">{gp.current}/{gp.target} เธเธญเธฃเนเธช</h3>
            ...
            <div className="group relative flex items-center gap-6 rounded-[2.5rem] bg-slate-50 p-8 border border-dashed border-slate-200 opacity-60">
              ...
              <div className="flex-1 text-left">
                ...
                <h3 className="text-xl font-bold text-slate-400 leading-none">เธขเธฑเธเนเธกเนเธกเธตเน€เธเนเธฒเธซเธกเธฒเธขเนเธเธเธ“เธฐเธเธตเน</h3>
              </div>
            </div>
          )}
        </div>
